...
### Get source code
#### Download sources
https://github.com/tattoopro/tattoopro/archive/master.zip

#### Or clone repository manually
```
git clone https://github.com/tattoopro/tattoopro.git
```
...
```
composer create-project tattoopro/tattoopro myproject.com
```
...
3. That's all - your application is accessible on [http://tattoopro.localhost](http://tattoopro.localhost)
...
MySQL is available on `tattoopro.localhost`, port `3306`. User - `root`, password - `root`
...
2. Adjust settings in `.env` file
	...
	```
	DB_DSN           = mysql:host=127.0.0.1;port=3306;dbname=tattoopro
	DB_USERNAME      = user
	...
	```
	FRONTEND_HOST_INFO    = http://tattoopro.localhost
	BACKEND_HOST_INFO     = http://backend.tattoopro.localhost
	STORAGE_HOST_INFP     = http://storage.tattoopro.localhost
	```
...
vagrant up
```
That`s all. After provision application will be accessible on http://tattoopro.localhost
...
Example of single domain config for nginx can be found [here](https://github.com/tattoopro/tattoopro/blob/master/docker/nginx/vhost_single_domain.conf)
