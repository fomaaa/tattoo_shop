...
# create nginx config
if [ ! -f /etc/nginx/sites-enabled/tattoopro.localhost ]; then
    sudo ln -s /var/www/vagrant/vhost.conf /etc/nginx/sites-enabled/tattoopro.localhost
fi

# Configuring application
echo "GRANT ALL PRIVILEGES ON *.* TO 'root'@'%' IDENTIFIED BY 'root'" | mysql -uroot -proot
echo "FLUSH PRIVILEGES" | mysql -uroot -proot
echo "CREATE DATABASE IF NOT EXISTS \`tattoopro\` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci" | mysql -uroot -proot
